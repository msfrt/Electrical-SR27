...
// such as frequency = 0, or a negative frequency, or a frequency faster than
// the clock-speed.
...
class EasyTimer {
 public:
  ...
  // member functions
  bool check();  // will check the timer to return true if time is up, or false
 ...
 private:
  ...
  bool enabled_ =
      true;  // used to enable and disable the timer (default enabled)
};

class LEDBlink {
 public:
  LEDBlink(GPIO_TypeDef* port, uint16_t pin, int freq);  // constructor
